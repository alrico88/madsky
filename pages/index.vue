<template lang="pug">
nav-bar(:pending="pending", has-refresh, @refresh="() => refresh()")
.grid.grid-cols-2(class="md:grid-cols-4 lg:grid-cols-6")
...
const { currPage, nextPage } = usePage("");

const { data, pending, refresh } = await useFetch("/api/measurements", {
  query: {
    page: currPage,
  },
  default: () => [],
